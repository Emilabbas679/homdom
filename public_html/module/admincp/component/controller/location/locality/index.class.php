...
class Admincp_Component_Controller_Location_Locality_Index extends AIN_Component
{
    public function process()
    {
        ...
        $aRows = AIN::getService('homdom.core')->homdom_get_locality($where);
        ...
        foreach ($aData as $iKey => $aRow)
        {
            $display = [];
            $tools = [];
            $tools[] = $table->buildMenu(AIN::getLib('url')->makeUrl('location.locality.edit', ['id' => $aRow['id']]), AIN::getPhrase('homdom.edit'), 'icon-pencil3 text-primary-600');
            $tools[] = $table->buildMenu(AIN::getLib('url')->makeUrl('location.locality.delete', ['id' => $aRow['id']]), AIN::getPhrase('homdom.delete'), 'icon-pencil3 text-primary-600');
            ...
            $display[AIN::getPhrase('homdom.district_name')] = AIN::getPhrase('homdom.'.$aRow['district_name']) ;
