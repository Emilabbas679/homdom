...
class Admincp_Component_Controller_Location_District_Add extends AIN_Component
{
    ...
    public function process()
    {
        ...
        if ($iEditId = $this->request()->getInt('id') and $iEditId != 0) {
            if ($aApi = AIN::getService('homdom.core')->homdom_get_district(['id' => $iEditId])) {
        ...
        if ($aVals = $this->request()->getArray('val')) {
            ...
            if (count($validation) > 0) {
                ...
                if ($bIsEdit) {
                    $aVals['id'] = $iEditId;
                    if (($data = AIN::getService('homdom.core')->homdom_update_district($aVals))) {
                        if ('success' == $data['status'])
                            $this->url()->send('location.district.index', ['id' => $aForms['id']], AIN::getPhrase('homdom.successfully_updated'));
                        else
                            AIN_ERROR::set(implode('<br/>', $data['messages']));
                    }
                } else {
                    if (($data = AIN::getService('homdom.core')->homdom_create_district($aVals))) {
                        if ('success' == $data['status']) {
                            $this->url()->send('location.district.index', ['id' => $aForms['id']], AIN::getPhrase('homdom.successfully_created'));
                        } else {
                            AIN_ERROR::set(implode('<br/>', $data['messages']));
                        }
                    }
        ...
        $aTableForm = [];
        $table = 0;
        $aTableForm[$table]['title'] = AIN::getPhrase('homdom.district_add');
        ...
        $aRows = AIN::getService('homdom.core')->homdom_get_region(['status_id' => 11, 'limit' => 100]);
        $options = [];
        ...
        foreach($options as $k=>$v){
            $aOptions[$v['id']] = AIN::getPhrase('homdom.').$v['phrase'];
        }

        $aTableForm[$table]['data'][] = [
            'title' => AIN::getPhrase('homdom.region'),
            'value' => (isset($aVals['region_id']) ? $aVals['region_id'] : (isset($aForms['region_id']) ? $aForms['region_id'] : '11')),
            'type' => 'select',
            'id' => 'region_id',
            'class' => 'region_id',
            'id_css' => 'region_id',
