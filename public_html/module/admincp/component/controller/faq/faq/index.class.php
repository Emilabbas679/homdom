...
class Admincp_Component_Controller_Faq_Faq_Index extends AIN_Component
{
    public function process()
    {
        ...
        $aRows = AIN::getService('homdom.core')->homdom_get_faq_faq($where);
        ...
        foreach ($aData as $iKey => $aRow)
        {
            ...
            $cat_title = (array) json_decode($aRow['category_title']);
            $display = [];
            $tools = [];
            $tools[] = $table->buildMenu(AIN::getLib('url')->makeUrl('faq.faq.edit', ['id' => $aRow['id']]), AIN::getPhrase('homdom.edit'), 'icon-pencil3 text-primary-600');
            ...
            $display[AIN::getPhrase('homdom.category_title')] = (isset($cat_title[$locale])) ? $cat_title[$locale] : "";
