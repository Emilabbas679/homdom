...
class Admincp_Component_Controller_Page_Index extends AIN_Component
{
    public function process()
    {
        ...
        $aRows = AIN::getService('homdom.core')->get_page($where);
        ...
        foreach ($aData as $iKey => $aRow)
        {
            $display = [];
            $tools = [];
            $tools[] = $table->buildMenu(AIN::getLib('url')->makeUrl('page.edit', ['id' => $aRow['id']]), AIN::getPhrase('homdom.edit'), 'icon-pencil3 text-primary-600');
            $tools[] = $table->buildMenu(AIN::getLib('url')->makeUrl('page.delete', ['id' => $aRow['id']]), AIN::getPhrase('homdom.delete'), 'icon-pencil3 text-primary-600');
            $display[AIN::getPhrase('homdom.id')] = $aRow['id'];
            $display[AIN::getPhrase('homdom.tools')] = $tools;
            $display[AIN::getPhrase('homdom.title')] = "<a style='color:blue;' href='https://homdom.az/page/".$aRow['slug']."' target='_blank'>".$aRow['title']."</a>";
