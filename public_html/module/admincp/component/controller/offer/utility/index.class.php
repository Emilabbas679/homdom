...
class Admincp_Component_Controller_Offer_Utility_Index extends AIN_Component
{
    public function process()
    {
        ...
        $aRows = AIN::getService('homdom.core')->homdom_get_offer_utility($where);
        ...
        foreach ($aData as $iKey => $aRow)
        {
            $display = [];
            $tools = [];
            $tools[] = $table->buildMenu(AIN::getLib('url')->makeUrl('offer.utility.edit', ['id' => $aRow['id']]), AIN::getPhrase('homdom.edit'), 'icon-pencil3 text-primary-600');
