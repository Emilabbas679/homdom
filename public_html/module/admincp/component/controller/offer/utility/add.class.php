...
class Admincp_Component_Controller_Offer_Utility_Add extends AIN_Component
{
    ...
    public function process()
    {
        ...
        if ($iEditId = $this->request()->getInt('id') and $iEditId != 0) {
            if ($aApi = AIN::getService('homdom.core')->homdom_get_offer_utility(['id' => $iEditId])) {
        ...
        if ($aVals = $this->request()->getArray('val')) {
            ...
            if (count($validation) > 0) {
                ...
                if ($bIsEdit) {
                    $aVals['id'] = $iEditId;
                    if (($data = AIN::getService('homdom.core')->homdom_update_offer_utility($aVals))) {
                        if ('success' == $data['status'])
                            $this->url()->send('offer.utility.index', ['id' => $aForms['id']], AIN::getPhrase('homdom.successfully_updated'));
                        else
                            AIN_ERROR::set(implode('<br/>', $data['messages']));
                    }
                } else {
                    if (($data = AIN::getService('homdom.core')->homdom_create_offer_utility($aVals))) {
                        if ('success' == $data['status']) {
                            $this->url()->send('offer.utility.index', ['id' => $aForms['id']], AIN::getPhrase('homdom.successfully_created'));
                        } else {
                            AIN_ERROR::set(implode('<br/>', $data['messages']));
                        }
                    }
        ...
        $aTableForm = [];
        $table = 0;
        $aTableForm[$table]['title'] = AIN::getPhrase('homdom.utility_add');
        ...
        $aOptions = [
            '1' => AIN::getPhrase('homdom.offer_property_type_id_1'),
            '2' => AIN::getPhrase('homdom.offer_property_type_id_2'),
            '3' => AIN::getPhrase('homdom.offer_property_type_id_3'),
            '5' => AIN::getPhrase('homdom.offer_property_type_id_5'),
            '6' => AIN::getPhrase('homdom.offer_property_type_id_6'),
            '4' =>AIN::getPhrase('homdom.offer_property_type_id_4')
        ];


        $aTableForm[$table]['data'][] = [
            'title' => AIN::getPhrase('homdom.property_type_id'),
            'value' => (isset($aVals['property_type_id']) ? $aVals['property_type_id'] : (isset($aForms['property_type_id']) ? $aForms['property_type_id'] : '1')),
            'type' => 'select',
            'id' => 'property_type_id',
            'class' => 'property_type_id',
            'id_css' => 'property_type_id',
            'options' => $aOptions,
            'required' => true,
        ];
