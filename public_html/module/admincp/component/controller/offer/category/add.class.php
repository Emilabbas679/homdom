...
class Admincp_Component_Controller_Offer_Category_Add extends AIN_Component
{
    ...
    public function process()
    {
        ...
        if ($iEditId = $this->request()->getInt('id') and $iEditId != 0) {
            if ($aApi = AIN::getService('homdom.core')->homdom_get_offer_property_category(['id' => $iEditId])) {
        ...
        if ($aVals = $this->request()->getArray('val')) {
            ...
            if (count($validation) > 0) {
                ...
                if ($bIsEdit) {
                    $aVals['id'] = $iEditId;
                    if (($data = AIN::getService('homdom.core')->homdom_update_offer_property_category($aVals))) {
                        if ('success' == $data['status'])
                            $this->url()->send('offer.category.index', ['id' => $aForms['id']], AIN::getPhrase('homdom.successfully_updated'));
                        else
                            AIN_ERROR::set(implode('<br/>', $data['messages']));
                    }
                } else {
                    if (($data = AIN::getService('homdom.core')->homdom_create_offer_property_category($aVals))) {
                        if ('success' == $data['status']) {
                            $this->url()->send('offer.category.index', ['id' => $aForms['id']], AIN::getPhrase('homdom.successfully_created'));
                        } else {
                            AIN_ERROR::set(implode('<br/>', $data['messages']));
                        }
                    }
