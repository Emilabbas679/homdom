...
class Admincp_Component_Controller_Offer_Purpose_Index extends AIN_Component
{
    public function process()
    {
        ...
        $aRows = AIN::getService('homdom.core')->homdom_get_offer_purpose($where);
        ...
        foreach ($aData as $iKey => $aRow)
        {
            $display = [];
            $tools = [];
            $tools[] = $table->buildMenu(AIN::getLib('url')->makeUrl('offer.purpose.edit', ['id' => $aRow['id']]), AIN::getPhrase('homdom.edit'), 'icon-pencil3 text-primary-600');
