...
class Admincp_Component_Controller_Complex_Block_Index extends AIN_Component
{
    public function process()
    {
        ...
        $aRows = AIN::getService('homdom.core')->get_complex_block($where);
        ...
        foreach ($aData as $iKey => $aRow)
        {
            ...
            $title = (isset($title[$locale])) ? $title[$locale] : "";
            $tools[] = $table->buildMenu(AIN::getLib('url')->makeUrl('complex.block.edit', ['id' => $aRow['id']]), AIN::getPhrase('homdom.edit'), 'icon-pencil3 text-primary-600');
            $tools[] = $table->buildMenu(AIN::getLib('url')->makeUrl('complex.block.delete', ['id' => $aRow['id']]), AIN::getPhrase('homdom.delete'), 'icon-pencil3 text-primary-600');
