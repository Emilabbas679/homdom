...
class homdom_component_controller_announce_add extends AIN_Component
{

    public function process()
    {
        $meta['title'] = AIN::getPhrase('homdom.new_offer') .' | '.$_SERVER['SERVER_NAME'];
        $this->template()->setTitle($meta['title']);
        $meta['description'] = AIN::getPhrase('homdom.new_offer_page_meta_description');
        ...
        if($aVals = $this->request()->getArray('val')) {
            ...
            if (isset($aVals['targets'])) {
                $targets = [];
                foreach ($aVals['targets'] as $val)
                    $targets[(string) $val] = $val;
            ...
            if (isset($sendApi['status']) and $sendApi['status'] == 'success') {
                AIN::getLib('url')->send('offer/'.$sendApi['data']['id']);
            }
            else{
...
//        print_r($aForms); die();
        ...
        if ($iEditId > 0) {
//            if ($aForms['building_id'] > 0) {
//                $building = AIN::getService('homdom.core')->homdom_get_building(['id' => $aForms['building_id']]);
//                if (isset($building['status']) and $building['status'] == 'success' and $building['data']['count'] == 1) {
//                    $aForms['building'] = $building['data']['rows'][0];
//                }
//            }
//            if ($aForms['region_id'] > 0) {
//                $region = AIN::getService('homdom.core')->homdom_get_region(['id' => $aForms['region_id']]);
//                if (isset($region['status']) and $region['status'] == 'success' and $region['data']['count'] == 1) {
//                    $aForms['region'] = $region['data']['rows'][0];
//                }
//            }
//            if ($aForms['metro_id'] > 0) {
//                $metro = AIN::getService('homdom.core')->homdom_get_metro(['id' => $aForms['metro_id']]);
//                if (isset($metro['status']) and $metro['status'] == 'success' and $metro['data']['count'] == 1) {
//                    $aForms['metro'] = $metro['data']['rows'][0];
//                }
//            }
//            if ($aForms['locality_id'] > 0) {
//                $metro = AIN::getService('homdom.core')->homdom_get_metro(['id' => $aForms['metro_id']]);
//                if (isset($metro['status']) and $metro['status'] == 'success' and $metro['data']['count'] == 1) {
//                    $aForms['metro'] = $metro['data']['rows'][0];
//                }
//            }
//            if ($aForms['district_id'] > 0) {
//                $district = AIN::getService('homdom.core')->homdom_get_district(['id' => $aForms['district_id']]);
//                if (isset($district['status']) and $district['status'] == 'success' and $district['data']['count'] == 1) {
//                    $aForms['city'] = $district['data']['rows'][0];
//                }
//            }
        }
