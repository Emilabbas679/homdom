...
class homdom_component_controller_offers extends AIN_Component
{
    ...
    public function process()
    {
        ...
        $offer_type = 0;
        $property_type = 0;
        if (isset($opt['offer_type']))
            $offer_type = $opt['offer_type'];
        if (isset($opt['property_type_id']))
            $property_type = $opt['property_type_id'];

        $title = AIN::getPhrase('homdom.offer_page_title_offer_type_'.$offer_type.'_property_type_'.$property_type);
        $meta['title'] = $title .' | '.$_SERVER['SERVER_NAME'];

        $this->template()->setTitle($meta['title']);

        $meta['description'] =  AIN::getPhrase('homdom.offer_page_meta_desc_offer_type_'.$offer_type.'_property_type_'.$property_type);
        ...
        AIN::getLib('pager')->set(['page' => $page, 'size' => 12, 'count' => $count]);
    }
