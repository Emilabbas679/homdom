...
function tb_show(caption, url, thisObject, sForceMessage, bForceNoCilck, sType)
{
	...
	if (urlType == '.jpg' || urlType == '.jpeg' || urlType == '.png' || urlType == '.gif' || urlType == '.bmp')
	{
		...
		imgPreloader.onload = function()
		{
			...
			sHtml += '</div><div class="js_box_close"><a href="#" onclick="return js_box_remove(this);">Bağla</a></div></div></div>';
	...
	if (!bIsAlreadyOpen)
	{
		...
		if (bIsPhotoImage){
			...
			sHtml += '<div class="js_box_close"><a href="#" onclick="return js_box_remove(this);">Bağla</a><span class="js_box_history">' + params[getParam('sGlobalTokenName') + '[call]'] + '</span></div>';
			sHtml += '</div>';
			sHtml += '</div>';
		}
		else{
