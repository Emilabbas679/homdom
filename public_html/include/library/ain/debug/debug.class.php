...
class AIN_Debug
{
	...
	{		
    	...
    	$oRequest = AIN::getLib('request');
    	$oFile = AIN::getLib('file');
    	$aReplaces = array_map(array('self', '_addKeywordSyntax'), $aKeywords);
    	$sDriver = AIN::getParam(array('db', 'driver'));
    	$sSql = '';
    	$bIsCmd = ((PHP_SAPI == 'cli' || (defined('AIN_IS_AJAX') && AIN_IS_AJAX)));
    	...
    	foreach(self::$_aDebugHistory['sql'] as $aLine)
    	{
    		...
    		$iSqlCount++;
    		$sExtra = AIN::getLib('database')->sqlReport($aLine['sql']);
    	...
    	// General Stats
    	$iTotalTime = sprintf('%0.7f', (array_sum(explode(' ', microtime()))-AIN_TIME_START));
    	$iTotalSqlTime = sprintf('%0.7f', $fSum);
    	
    	$sDebugReturn = '<div id="js_main_debug_holder">';
    	if (!defined('AIN_MEM_END'))
		{
			define('AIN_MEM_END', memory_get_usage());
		}
    	
    	if (AIN_DEBUG_LEVEL === 1)
    	{    	
    		$sDebugReturn .= '<div style="font-size:9pt; text-align:center; padding-bottom:50px;">Page generated in ' . round($iTotalTime, 4) . ' seconds with ' . $iSqlCount . ' queries and GZIP ' . (AIN::getParam('core.use_gzip') ? 'enabled' : 'disabled') . ' on ' . $_SERVER['SERVER_ADDR'] . '.</div>';
    	}    	
    	elseif (AIN_DEBUG_LEVEL === 2 || AIN_DEBUG_LEVEL === 3)
    	{
	    	$bSlaveEnabled = AIN::getParam(array('db', 'slave'));
	    	$aStats = array
	    	(
	    		// AIN
	    		'Version' => AIN::getVersion(),
	    		'Product Code Name' => AIN::getCodeName(),
	    		'1' => '',
	    		// Total Time
	    		'Total Time' => $iTotalTime,
	    		'PHP General Time' => ($iTotalTime - $iTotalSqlTime),
	    		'GZIP' => (AIN::getParam('core.use_gzip') ? 'enabled' : 'disabled'),
	    		'2' => '',
	    		// SQL
	    		'Driver Version' =>  ($sDriver == 'DATABASE_DRIVER' ? 'N/A' : AIN::getLib('database')->getServerInfo()),
	    		...
	    		'SQL Total Slaves' => ($bSlaveEnabled ? count(AIN::getParam(array('db', 'slave_servers'))) : 'N/A'),
	    		'SQL Slave Server' => ($bSlaveEnabled ? AIN::getLib('database')->sSlaveServer : 'N/A'),    		
	    		'3' => '',
	    		// Vars
	    		'Total Memory Usage' => $oFile->filesize(AIN_MEM_END),
	    		...
	    		'4' => '',
	    		'Load Balancing Enabled' => (AIN::getParam(array('balancer', 'enabled')) ? 'Yes' : 'No'),
	    		'Requests From' => $oRequest->getServer('SERVER_ADDR'),
	    		'Server ID#' => $oRequest->getServer('AIN_SERVER_ID'),
	    		'5' => '',
	    		'Server Time Stamp' => date('F j, Y, g:i a', AIN_TIME),
	    		'PHP Version' => PHP_VERSION,
	    		'PHP Sapi' => php_sapi_name(),
	    		'PHP safe_mode' => (AIN_SAFE_MODE ? 'true' : 'false'),
	    		'PHP open_basedir' => (AIN_OPEN_BASE_DIR ? 'true' : 'false'),
	    		'Operating System' => PHP_OS,
			'6' => '',
			'Cache'=> AIN::getParam('core.cache_storage')
	    	);
	    	...
	    	$aCookies = array();
	    	$sCookiePrefix = AIN::getParam('core.session_prefix');
	    	...
	    	{	    	
				...
				<div class="nDebugItem"><a href="#" onclick="if (getCookie(\'AIN_debug_detail\')) { deleteCookie(\'AIN_debug_detail\'); $(\'#AIN_debug_detail\').slideDown(); } else { $(\'#AIN_debug_detail\').slideUp(); setCookie(\'AIN_debug_detail\', \'1\', 365); } return false;">Debug Details</a></div>
				<div class="nDebugContent nDebugContentShell" id="AIN_debug_detail"' . (AIN::getCookie('AIN_debug_detail') ? ' style="display:none;"' : '') . '>
			...
			$sDebugReturn .= '<div class="nDebugItem">Debug History</div>
	    	...
	    	{
		    	$sDebugReturn .= '
				<div class="nDebugItem"><a href="#" onclick="if (getCookie(\'AIN_debug_session\')) { deleteCookie(\'AIN_debug_session\'); $(\'#AIN_debug_session\').slideDown(); } else { $(\'#AIN_debug_session\').slideUp(); setCookie(\'AIN_debug_session\', \'1\', 365); } return false;">Session</a></div>
				<div class="nDebugContent nDebugContentShell" id="AIN_debug_session"' . (AIN::getCookie('AIN_debug_session') ? ' style="display:none;"' : '') . '>
					<pre>'. self::_loadData($_SESSION[AIN::getParam('core.session_prefix')]) .'</pre>
				</div>
		
				<div class="nDebugItem"><a href="#" onclick="if (getCookie(\'AIN_debug_cookie\')) { deleteCookie(\'AIN_debug_cookie\'); $(\'#AIN_debug_cookie\').slideDown(); } else { $(\'#AIN_debug_cookie\').slideUp(); setCookie(\'AIN_debug_cookie\', \'1\', 365); } return false;">Cookie</a></div>
				<div class="nDebugContent nDebugContentShell" id="AIN_debug_cookie"' . (AIN::getCookie('AIN_debug_cookie') ? ' style="display:none;"' : '') . '>
    	...
    	if (defined('AIN_DEBUG_SHOW_FIXED'))
    	{
	...
	{
		$sStr = str_replace("\n", "<br />\n", trim($sStr));		
		$sStr = str_replace('UNIO_N ALL', "AIN_UNIO_N_ALL", $sStr);
		$sStr = str_replace('LEFT JOIN', "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;AIN_LEFT", $sStr);
		$sStr = str_replace('LEFT JOIN', "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;AIN_INNER", $sStr);
		$sStr = str_replace('JOIN', "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;JOIN", $sStr);
		$sStr = str_replace('AIN_LEFT', "LEFT JOIN", $sStr);
		$sStr = str_replace('AIN_INNER', "INNERT JOIN", $sStr);
		$sStr = str_replace('ON', "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;ON", $sStr);
		$sStr = str_replace('AIN_UNIO_N_ALL', "<br />UNION ALL<br />", $sStr);
		$sStr = trim($sStr);
		return $sStr;
	}
