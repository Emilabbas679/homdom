...
AIN::getLibClass('ain.database.interface');


abstract class AIN_Database_Dba implements AIN_Database_Interface
{
  ...
  public function singleData($file_name)
  {
	...
	{
		if (AIN_DEBUG && in_array(strtoupper($sAlias), $this->_aWords))
		{
			AIN_Error::trigger('The alias "' . $sAlias . '" is a reserved SQL word. Use another alias to resolve this problem.', E_USER_ERROR);
		}
	...
	public function table($sTable) {
		if (substr($sTable, 0, 1) == ':') {
			$sTable = AIN::getT(str_replace(':', '', $sTable));
		}
		return $sTable;
	}
	...
	public function limit($iPage, $sLimit = null, $iCnt = null, $bReturn = false, $bCorrectMax = false)
	{
		...
		if ($bCorrectMax == true)
		{
			$iOffset = ($iCnt === null ? $iPage : AIN_Pager::instance()->getOffset($iPage, $sLimit, $iCnt));
	 ...
	 * Example:
	 * <code>
	 * AIN::getLib('database')->updateCounter('user_count', 'total_friend', 'user_id', 1);
	 * </code>
	 *
	...
	{		
		$iCount = $this->select($sCounter)->from(AIN::getT($sTable))->where($sField . ' = ' . (int) $iId)->execute('getSlaveField');
		
		$this->update(AIN::getT($sTable), array($sCounter => ($bMinus === true ? (($iCount <= 0 ? 0 : $iCount - 1)) : ($iCount + 1))), $sField . ' = ' . (int) $iId);
	}	
	...
	public function updateCount($sCountTable, $aCountCond, $sCounter, $sUpdateTable, $aUpdateCond)
	{
		$iCount = $this->select('COUNT(*)')
			->from(AIN::getT($sCountTable))
			->where($aCountCond)
			->execute('getSlaveField');
			
		$this->update(AIN::getT($sUpdateTable), array($sCounter => $iCount), $aUpdateCond);
	}
	...
	public function searchKeywords($sField, $sStr)
	{
		...
		if (count($aWords))
		{
			...
			foreach ($aWords as $sWord)
			{
				...
				$sQuery .= $sField . ' LIKE \'%' . AIN::getLib('database')->escape($sWord) . '%\' ';
				...
				foreach ($aLikeWords as $sLikeWord)
				{
					...
					{
						$sQuery .= ' OR ' . $sField . ' LIKE \'%' . AIN::getLib('database')->escape($sLikeWord) . '%\'' ;
					}
				}
		...
		if (!$iIteration)
		{
			return $sField . ' LIKE \'%' . AIN::getLib('database')->escape($sStr) . '%\' ';
		}
  ...
  private function _singleData($data)
  {
	...
	*/
	...
	protected function _where($sKey, $mValue) {
		...
		$sWhere = 'AND ' . $sKey . ' = \'' . AIN::getLib('database')->escape($mValue) . '\' ';
	...
	protected function _join($sType, $sTable, $sAlias, $mParam = null)
	{
		...
		else 
		{
			...
			else 
			{
				...
				AIN_Error::trigger('Not allowed to use "USING()" in SQL queries any longer.', E_USER_ERROR);
			}
		}
