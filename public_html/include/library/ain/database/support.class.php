...
class AIN_Database_Support
{
	...
	public function getSupported($bReturnAll = false)
	{
		...
		{	
			...
			if (!@extension_loaded($aDbs['module']) 
				|| !file_exists(AIN_DIR_LIB_CORE . 'database' . AIN_DS . 'driver' . AIN_DS . $aDbs['driver'] . '.class.php')
	...
	public function getSchema($sName, $sPrefix = null)
	{
		...
		$sSchema = AIN_DIR_DEV . 'schema/' . $aDriver['schema'] . '.sql';
		
		if (!file_exists($sSchema))
		{
			return AIN_Error::set('Unable to load schema: ' . $sSchema);
		}
		...
		if ($sPrefix !== null)
		{
			$sContent = preg_replace('#AIN_#i', $sPrefix, $sContent);
		}
	...
	public function prepareSchema($aTables = array(), &$oDb = null)
	{
		...
		if ($aTables && is_array($aTables))
		{
			...
			{
				$aRows[$iKey]['Name'] = AIN::getT($sTable);
			}
		}
		else 
		{
		...
		foreach ($aRows as $aRow)
		{
			...
			$aRow['Name'] = str_replace(AIN::getParam(array('db', 'prefix')), 'AIN_', $aRow['Name']);
