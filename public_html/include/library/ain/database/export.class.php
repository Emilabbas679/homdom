...
class AIN_Database_Export
{
	...
	public function process($dbms, $aSchema)
	{
		...
		foreach ($aSchema as $table_name => $table_data)
		{
			...
			$sNoPrefix = str_replace(AIN::getParam(array('db', 'prefix')), '', $table_name);
	...
	public function processAlter($sDb, $aSql, $bIsPatch = false, $bInstall = false)
	{
		...
		if (isset($aSql['ADD_FIELD']))
		{
			foreach ($aSql['ADD_FIELD'] as $sTable => $aFields)
			{
				if ($bIsPatch)
				{
					$sTable = '\' . AIN::getT(\'' . preg_replace('#' . AIN::getParam(array('db', 'prefix')) . '#i', '', $sTable) . '\') . \'';					
				}
				else 
				{
					$sTable = preg_replace('#AIN_#i', AIN::getParam(array('db', 'prefix')), $sTable);
				}
				
				foreach ($aFields as $sField => $aExtras)
				{					
					if ($bInstall == true && AIN_Database::instance()->isField($sTable, $sField))
					{
						continue;
					}
					if ($bIsPatch)
		...
		if (isset($aSql['ADD_KEY']))
		{
			...
			{						
				...
				foreach ($aFields as $sField => $aExtras)
				{
					if ($bInstall == true && AIN_Database::instance()->isIndex($sTable, $sField))
					{
						continue;
					}
