...
class AIN_Url
{
	private $_aLanguages = array( 'az' => true, 'ru' => true, 'en' => true, 'tr' => true ) ;
	...
	private function _setParams()
	{
		if (AIN::getParam('core.url_rewrite') == 3)
		{
			...
			if ($aServer[0] == 'www')
			{
				$aServer[0] = AIN::getParam('core.module_core');
			}
			$_GET[AIN_GET_METHOD] = '/' . $aServer[0] . '/' . (isset($_GET[AIN_GET_METHOD]) ? $_GET[AIN_GET_METHOD] : '');			
		}
		elseif (AIN::getParam('core.url_rewrite') == 1)
		{
			$_GET[AIN_GET_METHOD] = $_SERVER['REQUEST_URI'];
		}	
		
		if (!isset($_GET[AIN_GET_METHOD]))
		{
			return '';
		}						
				
		if (AIN::getParam('core.url_rewrite') == 1)
		{
			$sRequest = parse_url($_GET[AIN_GET_METHOD], PHP_URL_PATH);
			$query  = parse_url($_GET[AIN_GET_METHOD], PHP_URL_QUERY);
		}
		else
		{
			$sRequest = $_GET[AIN_GET_METHOD];		
		}
		...
		foreach($aRequest as $sVar)
		{
			...
			if (!empty($sVar))
			{
				if(isset($this->_aLanguages[$sVar]) and $this->_aLanguages[$sVar] == true )
				{					
					$this->_aParams[AIN_GET_LANGUAGE] = $sVar;					
					continue;
				}						
				$iCnt++;
		...
		if (AIN::getParam('core.url_rewrite') == 1)
		{
			...
			{			
					...
					foreach($query2 as $aKey=>$aValue)
					{
						$this->_aParams[$aKey] = $aValue;
					}
			}			
		}	
	}	
	...
	public function getFullUrl($bNoPath = false)
	{
		if ($bNoPath)
		{
			return AIN::getLib('request')->get(AIN_GET_METHOD);
		}
	...
	{
		(($sPlugin = AIN_Plugin::get('url_getdomain_1')) ? eval($sPlugin) : false);
		...
		if (AIN::getParam('core.url_rewrite') != 3)
		{
			return AIN::getParam('core.path');	
		}		
		
		return (($this->_aParams['req1'] == AIN_MODULE_CORE) ? AIN::getParam('core.path') : preg_replace("/http:\/\/(.*?)\.(.*?)/i", "http://{$this->_aParams['req1']}.$1.$2", AIN::getParam('core.path')));
	}
	...
	public function cleanTitle($sTitle)
	{
		...
		{
			$sTitle = mb_substr($sTitle, 0, AIN::getParam('core.crop_seo_url'), 'UTF-8');			
		}
		else 
		{
			$sTitle = substr($sTitle, 0, AIN::getParam('core.crop_seo_url'));
		}
	...
	public function parseUrl($sUrl)
	{
		...
		switch (AIN::getParam('core.url_rewrite'))
		{
			case 1:				
				$aParts = explode(AIN::getParam('core.path'), $sUrl);
				$aParams = $this->_parseUrl($aParts[1]);				
				break;			
			case 2:				
				$aParts = explode(AIN_GET_METHOD . '=', $sUrl);
				$aParams = $this->_parseUrl($aParts[1]);				
				break;
			case 3:
				preg_match("/^http:\/\/(.*?)\.(.*?)\/(.*?)$/i", $sUrl, $aMatches);
				$sUrl = $aMatches[1] . '/' . str_replace(AIN::getParam('core.folder'), '', '/' . $aMatches[3]);
	...
	private function _send($sUrl, $iHeader = null)
	{
		...
		if (defined('AIN_IS_AJAX_PAGE') && AIN_IS_AJAX_PAGE)
		{
	...
	{		
		...
		{
			$sUrl = '';			
			foreach (array_merge($this->_aParams, $aParams) as $sKey => $sValue)
			{
				if (substr($sKey, 0, 3) == 'req')
				{
					$sUrl .= $sValue . '.';
		...
		{			
			if ( isset($aParams[AIN_GET_LANGUAGE]) and isset($this->_aLanguages[$aParams[AIN_GET_LANGUAGE]]) )
			{
				 $sUrl = $aParams[AIN_GET_LANGUAGE].'.' . $sUrl; 				 
			}
			elseif(isset($this->_aParams[AIN_GET_LANGUAGE]))
			{
				$sUrl = $this->_aParams[AIN_GET_LANGUAGE].'.' . $sUrl; 			
			}
			else
			{
				$sUrl = AIN_GET_LANGUAGE_VALUE.'.' . $sUrl; 			
			}					
		}
		$sUrls = '';
		switch (AIN::getParam('core.url_rewrite'))
		{
			...
			case 2:
				...
				$sUrls .= AIN::getParam('core.path') . AIN_INDEX_FILE . '?' . AIN_GET_METHOD . '=/';
				$sUrls .= $this->_makeUrl($aParts, $aParams);		
				break;
			// foo.site.com/bar/
			case 3:				
				...
				$sUrls = preg_replace("/http:\/\/(.*?)\.(.*?)/i", "http://{$aParts[0]}.$1.$2", AIN::getParam('core.path'));
				$sUrls .= $this->_makeUrl($aParts, $aParams);
				break;
		}
	...
	public function isRegistration($iReq)
	{
		...
		$sNextUrl = null;
		if (AIN::getLib('request')->get('req' . $iReq) == 'register' && is_array(AIN::getParam('user.registration_steps')) && count(AIN::getParam('user.registration_steps')))
		{
			$bIsRegistration = true;			
			$aUrls = AIN::getParam('user.registration_steps');
			$iLastKey = 0;
			foreach ($aUrls as $iKey => $sUrl)
			{
				if (AIN::getLib('module')->getFullControllerName() == $sUrl)
				{
					$iLastKey = $iKey;					
				}
			}
	...
	{		
		...
		$sUrl = AIN::getLib('url')->makeUrl($sLink, $aExtra);
	...
	{		 
		...
		{
			if (AIN::getParam('core.url_rewrite') == 3 && $iPartKey == 0)
			{
				continue;
			}
			...
			{
				$sPart = AIN::getParam('admincp.admin_cp');
			}            
		...
		if ($aParams && is_array($aParams))
		{
				...
				{	
					...
					{
						if( $sKey == AIN_GET_LANGUAGE )
						{
							continue;
						} 					
						//$bStr[$sKey] = str_replace('.', '', $sValue);						
						$bStr[$sKey] = $sValue;						
					}
				}	
				if ( isset($bStr) and count($bStr) > 0 )	
				{
					if(AIN::getParam('core.url_rewrite') == 2)
					{
						$sUrls .= '&'.urldecode(http_build_query($bStr, '', '&'));
					}
					else
					{
						$sUrls .= '?'.urldecode(http_build_query($bStr, '', '&'));
