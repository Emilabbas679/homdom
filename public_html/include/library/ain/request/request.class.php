...
class AIN_Request
{
	...
	{
		$this->_aArgs = $this->_trimData(array_merge($_GET, $_POST, $_FILES, AIN::getLib('url')->getParams()));				
	}
    	...
    	switch($sVar)
    	{
    		...
    		case 'AIN_SERVER_ID':
    			if (!AIN::getParam(array('balancer', 'enabled')))
    			{
					...
					{
						return AIN::getLib('cdn')->getServerId();
					}
    ...
    protected static $operatingSystems = 
		array(
        1        => 'Windows',
        2        => '\biPhone.*Mobile|\biPod|\biPad',
		3        => 'Android',

    );
    public function match($regex, $userAgent = null)
    {
        $match = (bool) preg_match(sprintf('#%s#is', $regex), (false === empty($userAgent) ? $userAgent : $_SERVER['HTTP_USER_AGENT']), $matches);
        return $match;
    }
	public function isOs($userAgent = null, $httpHeaders = null)
    {		
        foreach (self::$operatingSystems as $name => $_regex) {
	...
	public function isOsId($userAgent = null, $httpHeaders = null)
    {		
        foreach (self::$operatingSystems as $name => $_regex) {
            if ($this->match($_regex, $userAgent)) {
				return $name;
	...
	public function isDevice($userAgent = null, $httpHeaders = null)
	{
        foreach (self::$phoneDevices as $name => $_regex) {
            if ($this->match($_regex, $userAgent)) {
	...
	public function isIOS()
	{
 	...
 	{
 		//return $this->md5_hex_to_dec(md5((isset($_SERVER['HTTP_USER_AGENT']) ? $_SERVER['HTTP_USER_AGENT'] : null) . AIN::getParam('core.id_hash_salt') . (AIN::getParam('core.include_ip_sub_id_hash') ? $this->getSubstrIp($this->getIp()) : '')));
 		return (md5((isset($_SERVER['HTTP_USER_AGENT']) ? $_SERVER['HTTP_USER_AGENT'] : null) . AIN::getParam('core.id_hash_salt') . (AIN::getParam('core.include_ip_sub_id_hash') ? $this->getSubstrIp($this->getIp()) : '')));
 	}
 	...
 	*/
 	public function getSubstrIp($sIP, $iLength = null)
 	{
 		...
 		{
 			$iLength = AIN::getParam('core.ip_check');
 		}
	...
	*/
	...
	{
		return md5(AIN_TIME . AIN::getParam('core.path') . $this->getIdHash() . AIN::getParam('core.path') . rand(1, 1000000));
	}
 	/**
    ...
    public function getServerUrl($iId = '')
    {
		if (!$iId)
		{
			$iId = $this->getServer('AIN_SERVER_ID');
		}
	...
	public function send($sUrl, $aPost = array(), $sMethod = 'POST', $sUserAgent = null, $aCookies = null, $bFollow = false)
	{
		...
		if (extension_loaded('curl') && function_exists('curl_init'))
		{
			...
			curl_setopt($hCurl, CURLOPT_USERAGENT, ($sUserAgent === null ? "" . AIN::BROWSER_AGENT . " (" . AIN::getVersion() . ")" : $sUserAgent));
		...
		if (!isset($sData))
		{
	        ...
	        else
	        {
	            ...
	            $sSend .= "User-Agent: " . AIN::BROWSER_AGENT . " (" . AIN::getVersion() . ")\r\n";
