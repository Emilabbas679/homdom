...
{	
	...
	{		
		...
		require_once(AIN_DIR_SETTING . 'validator.sett.php');
		
		$this->_aRegex['user_name'] = '/^[a-zA-Z0-9_\- ]{' . AIN::getParam('user.min_length_for_username') . ',' . AIN::getParam('user.max_length_for_username') . '}$/';
		
		(($sPlugin = AIN_Plugin::get('validator_construct')) ? eval($sPlugin) : false);
	}
	...
	{		
		...
		{
			return AIN_Error::trigger('Invalid pattern.', E_USER_ERROR);
		}

		if (!preg_match($this->_aDefaults[$sPattern]['pattern'], $sValue))
		{
			return (isset($this->_aDefaults[$sPattern]['pattern']) ? AIN_Error::set($this->_aDefaults[$sPattern]['title']) : false);
		}
	...
	public function process($aParams, $aValues)
	{
		...
		{			
			...
			{				
				switch ($sType)
				{
					...
					case 'php_code:required':
						...
						if ($bFailed === false)
						{
							if (!empty($aValues[$sKey]))
							{
								$aValues[$sKey] = AIN::getLib('parse.format')->phpCode($aValues[$sKey]);	
							}
						}
					...
					default:						
						...
						{
							return AIN_Error::trigger('Not a valid validation type: ' . $sType, E_USER_ERROR);
						}
						break;
				}
				...
				if ($bFailed)
				{
					AIN_Error::set((isset($aParam['message']) ? $aParam['message'] : $sDebug . '(' . $sKey . ')') . ((AIN_DEBUG && isset($aParam['message'])) ? ' DEBUG: ' . $sDebug : ''));
				}				
			}
			...
			if ($bFailed === false)
			{
				...
				{
					$aValues[$sKey] = AIN::getLib('parse.input')->convert($aValues[$sKey]);
				}
			}
		}		
	...
	*/
	public function setAction($sAction)
	{
		...
		if (AIN::isModule($sModule) && AIN::hasCallback($sModule, 'getEnabledInputField'))
		{
			$aInputs = AIN::callback($sModule.'.getEnabledInputField');
			$aActions = array();
			...
			if (!empty($aActions))
			{
				...
				$aRequired = AIN_Database::instance()->select('i.field_id, i.phrase_var, ifc.table_name, ifc.column_name, ifc.operand, ifc.full_value')
					->from(AIN::getT('input_field'), 'i')
					->leftjoin(AIN::getT('input_field_condition'), 'ifc', 'ifc.field_id = i.field_id')
					->where('module_id = "'.$sModule .'" AND action IN ("'. implode('","',$aActions).'") AND is_required = 1')
					->execute('getSlaveRows');					
				$oInput = AIN::getService('input');
				foreach ($aRequired as $aInput)
				{
					if ($oInput->canEnterInfo($aInput))
					{
						$this->aFields['input_' . $aInput['field_id']] = array(
							'title' => 'Please provide a value for ' . AIN::getPhrase($aInput['phrase_var']), 
							'def' => 'required');
					}
				}
			}
	...
	{
		return ($bReturn ? 'return ' : '') . "\$Core." .$this->sFormName .'()' . ($bReturn ? ';' : '');
	}
	/**
	 * Client side check
	 * 
	...
	public function createJS()
	{
		...
		$sStr .= "\$Core." . $this->sFormName . " = function()\n{\n";
	 	$sStr .= "\t" . '$(\'#' . $this->_sName . '_msg\').hide(\'\').html(\'\');' . "\n";
	 	$sStr .= "\t" . "var bIsValid = true;\n";	 	
		$sStr .= "\t".'$(".error_message").each(function(){$(this).remove();});';
		$sStr .= "\t".'$(".alert_input").each(function(){$(this).removeClass("alert_input");});';
		...
		$sStr .= "\t" . "\n\t if ( bIsValid ) { return true; } \$('#" . $this->_sName . "_msg').show(\"slow\"); console.log('OK'); window.location.hash = '#pem'; return false;\n";
		$sStr .= '}' . "\n</script>\n";
		return $sStr;
	}
	...
	private function _checkRoutine($aFieldValue, $sFieldKey, $sType = 'php', $sValue = '')
	{
		...
		{
		    AIN_Error::set($aFieldValue['title']);
		}
	 	...
	 	elseif ( isset($aFieldValue['def']) )
	 	{
	 	 	if ($aFieldValue['def'] == 'required')
	 	 	{
				...
				{
					if (AIN::isModule('captcha') && !AIN::getService('captcha')->checkHash($sValue))
					{
						AIN_Error::set(AIN::getPhrase('captcha.captcha_failed_please_try_again'));
					}
				}
	 	 		else 
	 	 		{	 	 			
	 	 			if ($sType == 'php')
		 	 		{
		 	 			if (isset($aFieldValue['php_id']))
		 	 			{
		 	 				if (strpos($aFieldValue['php_id'], ']'))
		 	 				{
		 	 					...
		 	 					$aPostArray = AIN_Request::instance()->getArray($aParts[0]);
		 	 			...
		 	 			elseif (is_string($sValue) && AIN::getLib('parse.format')->isEmpty($sValue))
		 	 			{		 	 						 	 				
		 	 				AIN_Error::set($aFieldValue['title']);
		 	 			}
		 	 		...
		 	 		{
		 	 			(($sPlugin = AIN_Plugin::get('validator_check_routine_default')) ? eval($sPlugin) : false);
		 	 			...
		 	 			if (!isset($bSkipDefaultCheck))
		 	 			{
			 	 			...
			 	 			{
			 	 				$sStr .= $this->_createIfJS('' . (($sFieldKey == 'text' && AIN::getParam('core.wysiwyg') == 'tinymce') ? '(Editor.sEditor == \'tinymce\' && typeof(tinyMCE) == \'object\' && tinyMCE.activeEditor.getContent().replace(/<\/?[^>]+>/gi, \'\').length == 0) || (typeof(tinyMCE) != \'object\' && $(\'#'. $sFieldKey .'\').val() == \'\') || (Editor.sEditor != \'tinymce\' && typeof(tinyMCE) == \'object\' && $(\'#'. $sFieldKey .'\').val() == \'\')' : '$(\'#'. $sFieldKey .'\').val() == \'\'') . '', $aFieldValue['title'], $sFieldKey);
			 	 			}
		 	 			}
		 	 		}
	 	 		}
