...
class AIN_File
{
	 ...
	 * Example:
	 * <code>
	 * $aMeta = AIN_File::instance()->getMeta('/var/www/sample.jpg');
	 * </code>
	...
	public function getMeta($sFileName)
	{
		...
		require_once(AIN_DIR_LIB . 'getid3' . AIN_DS . 'getid3' . AIN_DS . 'getid3.php');
	...
	public function setMeta($sFileName, $TagData)
	{
		AIN_Error::skip(true);	
		
		require_once(AIN_DIR_LIB . 'getid3' . AIN_DS . 'getid3' . AIN_DS . 'getid3.php');

		require_once(AIN_DIR_LIB . 'getid3' . AIN_DS . 'getid3' . AIN_DS . 'write.php');

		$tagwriter = new getid3_writetags;
		$tagwriter->filename = $sFileName;
	...
	public function load($sFormItem, $aSupported = array(), $iMaxSize = null)
	{
		...
		{
			return AIN_Error::set(AIN::getPhrase('core.not_a_valid_file_extension_we_only_accept_support', array('support' => implode(', ', $aSupported))));
		}
		
		if (AIN::getLib('image')->isImageExtension($this->_aFile['ext']) && !AIN::getLib('image')->isImage($this->_aFile['tmp_name']))
		{
			return AIN_Error::set(AIN::getPhrase('core.not_a_valid_image_we_only_accept_the_following_file_extensions_support', array('support' => implode(', ', $aSupported))));
		}
    ...
    public function upload($sFormItem, $sDestination, $sFileName, $bModifyFileName = true, $iPerm = 0644, $buildDir = true, $bCdn = true)
    {
        ...
        {
        	$sFileName = md5($sFileName . AIN_TIME . uniqid());
        }
		...
		$this->_sDestDir = $this->_sDestination . $sFileName . '.' . $this->_sExt;
	
		if (file_exists($sFormItem))
		{
			@copy($sFormItem, $this->_sDestDir);
		}
	    else if ( AIN::getLib('request')->method() == 'PUT' )
		{
			@copy($this->_aFile['tmp_name'], $this->_sDestDir);
			@unlink($this->_aFile['tmp_name']);				
		}
		else if (!@move_uploaded_file($this->_aFile['tmp_name'], $this->_sDestDir))
        {
            return AIN_Error::set(AIN::getPhrase('core.unable_to_move_the_file'));
        }	
        if (stristr(PHP_OS, "win"))
        {
        	@copy($this->_sDestDir, $this->_sDestDir . '.cache');
        	@unlink($this->_sDestDir);
        	@copy($this->_sDestDir . '.cache', $this->_sDestDir);
        	@unlink($this->_sDestDir . '.cache');
        }
        else
        {
			@chmod($this->_sDestDir, $iPerm);
        }
	...
	{		
        ...
        if ($hDir = @opendir($sDir))
        {
			while (false !== ($sFile = readdir($hDir)))
           	{
            	if ($sFile == '.' || $sFile == '..' || $sFile == '.svn' || $sFile == '.svn-ignore')
               	{
					continue;
               	}
               	$aFiles[] = $sFile;
	...
	public function getAllFiles($sDir, $bRecurse = false)
	{
		...
		while ($sFile = readdir($hDir))
		{
			...
			$sNewDir = rtrim($sDir, AIN_DS) . AIN_DS . $sFile;
	...
	{		
		if ($hFile = @fopen(AIN_DIR_CACHE . $sFile, 'w+'))
		{			
			fwrite($hFile, $sData);
			fclose($hFile);		
		}
	...
	{	    
		...
		if (AIN::getParam('core.allow_cdn') && $iServerId > 0)
		{
			//$sFile = AIN::getLib('cdn')->getUrl(str_replace(AIN_DIR, AIN::getParam('core.path'), $sFile), $iServerId);					
	    ...
	    {
	    	$sServer = AIN_Request::instance()->getServerUrl($iServerId);
	    	$sFileServer = $sServer . '/' .str_replace(AIN_DIR, '', $sFile);
    ...
    {    	
        ...
        if (stristr(PHP_OS, "win") || $bForce === true)
        {
            ...
            if (is_dir($sPath))
            {
                 ...
                 */
                $fp = @fopen($sPath . AIN_DS . 'win-test.txt', 'w');                
                if (!$fp)
                {
                    if ($bForce === true)
    				{
    ...
    {		
		...
		return rtrim($sTempDir, AIN_DS) . AIN_DS;	
    }
	...
	{		
		if ($bRecurse === true)
		{			
			$aParts = explode(AIN_DS, trim($sDir, AIN_DS));
			$sParentDirectory = (AIN::getLib('server')->isWindows() ? '' : AIN_DS);
    ...
    private function _passLimit()
    {
		...
		*/
		...
		{
			//return AIN_Error::set('Image dimensions too small');
		}
    	switch ($this->_aFile['error'])
    	{
    		case 1:
    				return AIN_Error::set(AIN::getPhrase('core.upload_failed_server_cannot_handle_files_larger_then_file_size', array('file_size' => ini_get('upload_max_filesize'))));
    			break;
    		default:
	    			$iSize = filesize($this->_aFile['tmp_name']);
	    			$iPostMaxSize = ((int) ini_get('post_max_size') * 1048576);		
					
					//return AIN_Error::set("$iSize >= $iPostMaxSize");
			    	if ($iSize >= $iPostMaxSize)
			    	{
						////return AIN_Error::set(" iSize : $iSize , iPostMaxSize : $iPostMaxSize");						
			    		return AIN_Error::set(AIN::getPhrase('core.upload_failed_server_cannot_handle_files_size_larger_then_file_size', array('size' => $this->filesize($iSize, 0), 'file_size' => $this->filesize($iUploadMaxFileSize))));
			    	}    
			    	
			    	if ($this->_iMaxSize !== null && $iSize >= ($this->_iMaxSize * 1048576))
			    	{
			    		
			    		return AIN_Error::set(AIN::getPhrase('core.upload_failed_your_file_size_is_larger_then_our_limit_file_size', array('size' => $this->filesize($iSize, 0), 'file_size' => $this->filesize(($this->_iMaxSize * 1048576)))));
			    	}
	...
	{
		if (!AIN_SAFE_MODE && AIN::getParam('core.build_file_dir') && !defined('AIN_IS_HOSTED_SCRIPT'))
		{
			$aParts = explode('/', AIN::getParam('core.build_format'));
			$this->_sDestination = $sDestination;
			foreach ($aParts as $sPart)
			{
				$sDate = date($sPart) . AIN_DS;
