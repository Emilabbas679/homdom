...
class AIN_Parse_Format
{
	...
	{		
		...
		if (preg_match("/&\#160;/i", AIN::getLib('parse.input')->clean($sStr)) && strlen(preg_replace_callback("/&\#160;/is", array($this, '_checkIfEmpty'), AIN::getLib('parse.input')->clean($sStr))) === 0)
		{
			$bEmpty = true;
		}	
	 ...
	 * Usage:
	 * <code>
	 * echo AIN::getLib('parse.format')->hideEmail('foo@bar.com');
	...
	function compact_number_format($number)
	{
		if ($number < 10000)
		{
