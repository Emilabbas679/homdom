...
class AIN_Parse_Output
{
	...
	{		
		...
		if ($sPlugin = AIN_Plugin::get('AIN_parse_output_parse__start')){eval($sPlugin);}
		$sTxt = ' ' . $sTxt;
		
		if (!AIN::getParam('core.allow_html'))
		{
			$oFilterBbcode = AIN::getLib('parse.bbcode');
			
			if (AIN::isModule('emoticon'))
			{
				$sTxt = AIN::getService('emoticon')->parse($sTxt);
			}
		...
		if (AIN::getParam('core.xhtml_valid'))
		{
			$sTxt = AIN::getLib('parse.format')->validXhtml($sTxt);
		}
		...
		$sTxt = preg_replace_callback('/\[AIN_PHRASE\](.*?)\[\/AIN_PHRASE\]/i', array($this, '_getPhrase'), $sTxt);
		
		if (AIN::getParam('core.resize_images'))
		{
	...
	{
		if (AIN::getParam('core.disable_all_external_emails'))
		{
			$sTxt = preg_replace_callback('#([\s>])([.0-9a-z_+-]+)@(([0-9a-z-]+\.)+[0-9a-z]{2,})#i', array(&$this, '_replaceEmails'), $sTxt);	
		}
		
		if (AIN::getParam('core.disable_all_external_urls'))
		{
		...
		if (AIN::getParam('core.no_follow_on_external_links'))
		{
			$sTxt = preg_replace('/<a\s(.*?)>/is', '<a \\1 rel="nofollow">', $sTxt);
		}		
		
		if (AIN::getParam('core.replace_url_with_links') || AIN::getParam('core.warn_on_external_links') || AIN::getParam('core.no_follow_on_external_links'))
		{		
	...
	*/
	private function _parseUserTagged($iUser)
	{
		...
		{
			$oDb = AIN::getLib('database');
	
			$aUser = $oDb->select('up.user_value, u.full_name, user_name')
				->from(AIN::getT('user'), 'u')
				->leftjoin(AIN::getT('user_privacy'), 'up', 'up.user_id = u.user_id AND up.user_privacy = \'user.can_i_be_tagged\'')
			...
			{
				$sOut = '<a href="' . AIN::getLib('url')->makeUrl($aUser['user_name']) .'">' . $aUser['full_name'] .'</a>';
			}
	...
	public function feedStrip($sStr)
	{
		...
		if (!AIN::getParam('core.allow_html_in_activity_feed'))
		{
			$sId = uniqid();
			$sStr = preg_replace('/<img src="' . preg_quote(AIN::getParam('core.path'), '/') . 'file\/pic\/emoticon\/(.*?)"(.*?)\/>/is', '[EMOTICON' . $sId . ']\\1[/EMOTICON]', $sStr);
						
			$sStr = strip_tags($sStr, '<br ><br>');			
			
			$sStr = preg_replace('/\[EMOTICON' . $sId . '\](.*?)\[\/EMOTICON\]/is', '<img src="' . AIN::getParam('core.path') . 'file/pic/emoticon/\\1" class="v_middle" \/>', $sStr);
		}
		
		$sStr = $this->parseUrls($sStr);		
		if (AIN::isModule('emoticon'))
		{
			$sStr = AIN::getService('emoticon')->parse($sStr);
		}
		...
		$sStr = preg_replace_callback('/\[AIN_PHRASE\](.*?)\[\/AIN_PHRASE\]/i', array($this, '_getPhrase'), $sStr);
	...
	public function setEmbedParser($aParams = null)
	{
		if ($aParams == null)
		{
			$this->_aEmbedParams = array('width' => AIN::getParam('feed.width_for_resized_videos'), 'height' => AIN::getParam('feed.height_for_resized_videos'));
		}
    ...
    public function shorten($html, $maxLength, $sSuffix = null, $bHide = false)
    {
	    ...
	    $position = 0;
	    $tags = array();
		$html = AIN::getLib('parse.bbcode')->removeTagText($html);
	    $sNewString = '';
		...
		if ($sSuffix !== null)
		{
			...
			{
				if (preg_match('/^(.*)\.(.*)$/', $sSuffix, $aMatches) && count($aMatches) === 3 && AIN::isModule($aMatches[1]))
				{
					$sSuffix = AIN::getPhrase($sSuffix);
				}			
				...
				if ($bHide === true)
				{
			    	...
			    	$sNewString .= $html;
			    	$sNewString .= '<div class="item_view_more"><a href="#" onclick="$(this).parents(\'.js_view_more_parent:first\').find(\'.js_view_more_full\').hide(); $(this).parents(\'.js_view_more_parent:first\').find(\'.js_view_more_part\').show(); return false;">' . AIN::getPhrase('core.view_less'). '</a></div>';
			    	$sNewString .= '</span>';
			    	$sNewString .= '</span>';		    
				}
				else 
    ...
    { 	
    	...
    	$sNewString = '';
   		...
   		$sOut = ($bHasNonAscii === true ? str_replace(array('[', ']'), array('&lt;', '&gt;'), AIN::getLib('parse.input')->convert($sNewString)) : $sNewString);
	...
	private function _replaceEmails($aMatches)
	{
		...
		if ($aSites === null)
		{
			$aSites = explode(',', trim(AIN::getParam('core.email_white_list')));
		}
	...
	private function _replaceLinks($aMatches)
	{
		...
		if ($aSites === null)
		{
			$aSites = explode(',', trim(AIN::getParam('core.url_spam_white_list')) . ',' . AIN::getParam('core.host'));
		}
    ...
    {    	
		...
		$sName = $aMatches[2];		
		if (AIN::getParam('core.shorten_parsed_url_links') > 0)
		{
			$sName = substr($sName, 0, AIN::getParam('core.shorten_parsed_url_links')) . (strlen($sName) > AIN::getParam('core.shorten_parsed_url_links') ? '...' : '');
		}		
		
		if (AIN::getParam('core.warn_on_external_links'))
		{
			if (!preg_match('/' . preg_quote(AIN::getParam('core.host')) . '/i', $sHref))
			{
				$sHref = AIN::getLib('url')->makeUrl('core.redirect', array('url' => AIN::getLib('url')->encode($sHref)));
			}
		}	
    ...
    {    	
    	if (AIN::getParam('core.resize_embed_video'))
    	{       		
    ...
    {
    	return (isset($aMatches[1]) ? AIN::getPhrase($aMatches[1]) : $aMatches[0]);
    }
    ...
    {
    	$aParts = AIN::getLib('parse.input')->getParams($aMatches[1]);
    	...
    	(($sPlugin = AIN_Plugin::get('parse_output_fiximagewidth')) ? eval($sPlugin) : false);
